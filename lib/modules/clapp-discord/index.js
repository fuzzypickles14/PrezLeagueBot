...
class App extends Clapp.App {
  ...
  _getHelp() {
    ...
    var table = new Table({
      ...
      border: [],
      colWidths: [
        Math.round(0.35*LINE_WIDTH), // We round it because providing a decimal number would
        Math.round(0.35*LINE_WIDTH)  // break cli-table2
      ],
      wordWrap: true
    });

    for (let i in this.commands) {
      table.push([i.toString(), this.commands[i].desc]);
    }
    r +=
      '```' + table.toString() + '```\n\n' +
